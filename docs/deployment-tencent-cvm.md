...
- 推荐快速迭代：PM2 + Caddy，不构建 Docker 镜像，部署速度更快。
- 容器化交付：Docker Compose + Caddy，服务隔离更强，但首次构建会更慢。

Docker Compose 路线通过一份 `compose.yaml` 在同一台腾讯云 CVM 上运行以下服务：
...
6. 二选一安装运行环境：
   - PM2 路线：脚本会自动检查并安装 Node.js 24、pnpm、PM2、Caddy。
   - Docker 路线：安装 Docker Engine 和 Docker Compose 插件。
...
推荐直接使用一键部署脚本生成 `.env`、随机密钥并启动服务。

PM2 路线：

```bash
deploy/pm2/deploy.sh --site-host www.example.com --email admin@example.com
```

Docker Compose 路线：

```bash
deploy/tencent-cvm/deploy.sh --site-host www.example.com --email admin@example.com
```
...
# PM2 路线建议值；Docker 路线由 compose.yaml 注入容器内路径。
DATABASE_PATH=/srv/anshow/data/anshow.db
LOCAL_MEDIA_ROOT=/srv/anshow/media
BACKUP_DIR=/srv/anshow/backups
```
...
使用一键部署脚本时，第 2 步已经完成启动、迁移、健康检查和访问地址输出。
...
- PM2 路线：`anshow-backend`、`anshow-worker`、`anshow-frontend` 状态为 online。
- Docker 路线：`migrate` 退出码为 0，`backend` 和 `frontend` 状态为 healthy。
...
pm2 stop anshow-frontend anshow-backend anshow-worker

test -f "/srv/anshow/backups/.staging/restore-${RUN_ID}/anshow.db"
cp -a /srv/anshow/data/anshow.db "/srv/anshow/data/anshow.db.pre-restore"
for suffix in -wal -shm; do
  if [ -f "/srv/anshow/data/anshow.db${suffix}" ]; then
    cp -a "/srv/anshow/data/anshow.db${suffix}" "/srv/anshow/data/anshow.db${suffix}.pre-restore"
  fi
done
tar -czf /srv/anshow/data/media.pre-restore.tgz -C /srv/anshow/media .
rm -f /srv/anshow/data/anshow.db-wal /srv/anshow/data/anshow.db-shm
cp "/srv/anshow/backups/.staging/restore-${RUN_ID}/anshow.db" /srv/anshow/data/anshow.db
if [ -d "/srv/anshow/backups/.staging/restore-${RUN_ID}/media" ]; then
  rm -rf /srv/anshow/media/*
  cp -a "/srv/anshow/backups/.staging/restore-${RUN_ID}/media/." /srv/anshow/media/
fi

pnpm --filter @anshow/backend db:migrate
pm2 start anshow-backend anshow-worker anshow-frontend
curl -fsS "https://${SITE_HOST}/api/health/ready"
unset RUN_ID
```
...
```bash
git pull --ff-only
deploy/pm2/deploy.sh --skip-install
pm2 status
curl -fsS "https://${SITE_HOST}/api/health/ready"
```
...
```bash
pm2 status
pm2 logs anshow-backend --lines 300 --nostream
pm2 logs anshow-worker --lines 300 --nostream
pm2 logs anshow-frontend --lines 300 --nostream
pm2 restart anshow-worker
systemctl status caddy --no-pager
journalctl -u caddy -n 300 --no-pager
```
