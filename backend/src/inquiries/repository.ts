  ...
  return {
    createWithNotification(input: NewInquiryInput) {
      ...
      return db.transaction((tx) => {
        ...
        const row = tx.insert(inquiries).values({id,name:input.name,company:input.company,email:input.email,phone:input.phone,transportNeed:input.transportNeed,message:input.message,locale:input.locale,sourceUrl:input.sourceUrl,referrer:input.referrer??null,utmSource:input.utmSource??null,utmMedium:input.utmMedium??null,utmCampaign:input.utmCampaign??null,privacyVersion:input.privacyVersion,consentedAt:now,priority:"normal",status:"new",createdAt:now,updatedAt:now,closedAt:null}).returning().get();
    ...
    updateStatus(id:string, status:InquiryStatus, actorId:string) { return db.transaction(tx=>{ const current=tx.select().from(inquiries).where(eq(inquiries.id,id)).get(); if(!current) throw new Error("INQUIRY_NOT_FOUND"); assertInquiryTransition(current.status as InquiryStatus, status); const now=Date.now(); const closedAt=status==="closed"||status==="spam"?now:null; tx.update(inquiries).set({status,updatedAt:now,closedAt}).where(eq(inquiries.id,id)).run(); tx.insert(inquiryHistory).values({id:crypto.randomUUID(),inquiryId:id,actorId,assigneeId:current.assigneeId,fromStatus:current.status,toStatus:status,createdAt:now}).run(); return {...current,status,updatedAt:now,closedAt}; }); },
    assign(id:string, assigneeId:string|null, actorId:string) { return db.transaction(tx=>{ const current=tx.select().from(inquiries).where(eq(inquiries.id,id)).get(); if(!current) throw new Error("INQUIRY_NOT_FOUND"); const now=Date.now(); tx.update(inquiries).set({assigneeId,updatedAt:now}).where(eq(inquiries.id,id)).run(); tx.insert(inquiryHistory).values({id:crypto.randomUUID(),inquiryId:id,actorId,assigneeId,fromStatus:current.status,toStatus:current.status,createdAt:now}).run(); return {...current,assigneeId,updatedAt:now}; }); },
    addNote(inquiryId:string, authorId:string, body:string) { return db.transaction(tx=>{ const now=Date.now(); const current=tx.select({id:inquiries.id}).from(inquiries).where(eq(inquiries.id,inquiryId)).get(); if(!current) throw new Error("INQUIRY_NOT_FOUND"); const note=tx.insert(inquiryNotes).values({id:crypto.randomUUID(),inquiryId,authorId,body,createdAt:now}).returning().get(); tx.update(inquiries).set({updatedAt:now}).where(eq(inquiries.id,inquiryId)).run(); return note; }); },
