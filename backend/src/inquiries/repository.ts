...
import type { InquiryStatus } from "./state-machine.js";
...
export function createInquiryRepository(db: AppDatabase) {
  return {
    ...
    list(filters: {status?: InquiryStatus; assigneeId?: string; limit?: number; offset?: number} = {}) {
      const conditions = [filters.status ? eq(inquiries.status,filters.status) : undefined, filters.assigneeId ? eq(inquiries.assigneeId,filters.assigneeId) : undefined].filter(Boolean) as any[];
      let query = db.select().from(inquiries).orderBy(asc(inquiries.createdAt)).limit(filters.limit??50).offset(filters.offset??0);
    ...
    updateStatus(id:string, status:InquiryStatus, actorId:string) { return db.transaction(tx=>{ const current=tx.select().from(inquiries).where(eq(inquiries.id,id)).get(); if(!current) throw new Error("INQUIRY_NOT_FOUND"); tx.update(inquiries).set({status}).where(eq(inquiries.id,id)).run(); tx.insert(inquiryHistory).values({id:crypto.randomUUID(),inquiryId:id,actorId,assigneeId:current.assigneeId,fromStatus:current.status,toStatus:status,createdAt:Date.now()}).run(); return {...current,status}; }); },
    assign(id:string, assigneeId:string|null, actorId:string) { db.update(inquiries).set({assigneeId}).where(eq(inquiries.id,id)).run(); db.insert(inquiryHistory).values({id:crypto.randomUUID(),inquiryId:id,actorId,assigneeId,toStatus:"contacted",createdAt:Date.now()}).run(); },
