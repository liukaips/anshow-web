...
import { StaffRepositoryError, type StaffRepository } from "../repositories/staff-repository.js";
...
const rolesMutation = createRoute({ method: "post", path: "/api/admin/staff/{id}/roles", operationId: "updateStaffRoles", request: { params: idParam, body: { content: { "application/json": { schema: z.object({ roleIds: z.array(z.string()).max(20) }) } } } }, responses: { 200: { description: "Updated", content: { "application/json": { schema: envelope(z.object({ updated: z.literal(true) })) } } }, 409: { description: "Conflict", content: { "application/json": { schema: error } } } } });
...
export function registerStaffRoutes(app: OpenAPIHono<AppEnv>, repository: StaffRepository, permissions: PermissionMiddlewareDependencies) {
  ...
  app.openapi(mutation("disableStaff", "disable"), (c) => { const id = c.req.valid("param").id; try { repository.disable(id, c.get("actor")!.user.id); return c.json({ data: { updated: true as const }, error: null, requestId: c.get("requestId") }, 200); } catch (e) { if (e instanceof StaffRepositoryError) return c.json({ data: null, error: { code: e.code, message: e.message }, requestId: c.get("requestId") }, 409); throw e; } });
  ...
  app.openapi(rolesMutation, (c) => { const { id } = c.req.valid("param"); try { repository.setRoles(id, c.req.valid("json").roleIds, c.get("actor")!.user.id); return c.json({ data: { updated: true as const }, error: null, requestId: c.get("requestId") }, 200); } catch (e) { if (e instanceof StaffRepositoryError) return c.json({ data: null, error: { code: e.code, message: e.message }, requestId: c.get("requestId") }, 409); throw e; } });
}
